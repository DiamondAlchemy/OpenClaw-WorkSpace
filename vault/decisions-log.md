...
- **2026-06-24** — Diamond approved upgrading and locking down `gogcli` after the gogcli.sh review. MoneyPenny tapped and trusted `openclaw/tap/gogcli`, reinstalled `gogcli`, and upgraded the local `gog` binary from v0.11.0 to v0.31.0. New safety controls verified include global `--readonly`, `--gmail-no-send`, and typed read-only MCP tool discovery. Remaining blockers: macOS Keychain requires an explicit "Always Allow" approval for the upgraded binary to read existing OAuth tokens, and Calendar API enablement failed because the available gcloud accounts lack permission on project `427375316869`.
